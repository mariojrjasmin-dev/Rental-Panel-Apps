...
type AuthContextType = {
  ...
  register: (name: string, email: string, password: string, termsAccepted?: boolean, adultConfirmed?: boolean) => Promise<void>;
  logout: () => Promise<void>;
...
export default function RootLayout() {
  ...
  const register = async (name: string, email: string, password: string, termsAccepted: boolean = false, adultConfirmed: boolean = false) => {
    const res = await fetch(`${BACKEND_URL}/api/auth/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, email, password, terms_accepted: termsAccepted, adult_confirmed: adultConfirmed }),
    });
    if (!res.ok) {
      const err = await res.json();
