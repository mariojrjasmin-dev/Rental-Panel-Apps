...
backend:
  - task: "Forgot Password (6-digit code) + Register requires terms_accepted=true (POST /api/auth/forgot-password, POST /api/auth/reset-password, POST /api/auth/register with terms_accepted field)"
    implemented: true
    working: true
...
frontend:
  ...
  - task: "Replace 'DAMS Car Rental' text headers with the brand logo (receipt screen, PDF receipt, public legal pages)"
    implemented: true
    working: true
    file: "frontend/app/receipt.tsx, backend/server.py"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
        - working: true
          agent: "main"
          comment: "✅ END-TO-END VERIFIED. Three customer-facing places previously displaying 'DAMS' / 'DAMS CAR RENTAL' / 'DAMS RENT A CAR, S.R.L.' as text now display the actual brand logo image (dams-logo.png). (1) MOBILE RECEIPT SCREEN HEADER (frontend/app/receipt.tsx lines 145-148): The `<Text style={styles.headerBrand}>DAMS</Text>` was replaced with `<BrandLogo size='small' imageStyle={width:100,height:38} />` keeping the 'RECEIPT' red caption below. Verified via screenshot (/tmp/receipt-logo.png): logo renders crisp at 100×38 in the center of the receipt header. (2) PDF RECEIPT HEADER (backend/server.py lines 1483-1500): replaced the text drawing routines with `c.drawImage(ImageReader(LOGO_PATH), 18mm, H-22mm, width=auto, height=14mm, preserveAspectRatio=True, mask='auto')`; falls back to the old text if the logo file is missing. PDF size went from ~2.8KB → ~34KB confirming the PNG is embedded. Rendered to JPG via pdftoppm — logo appears clearly in the dark top bar with 'RECEIPT' on the right. (3) PUBLIC LEGAL HTML PAGES (backend/server.py _legal_layout function, lines ~2920-2960): replaced the orange 'DR' badge + `<h1>DAMS RENT A CAR, S.R.L.</h1>` + subtitle with a single `<img src='/api/assets/logo.png' />` at 56px tall. Verified both /api/legal/terms and /api/legal/privacy render the logo correctly. (4) NEW PUBLIC ENDPOINT: GET /api/assets/logo.png streams the brand PNG from /app/frontend/assets/images/dams-logo.png with Cache-Control: public, max-age=86400. Returns 200 image/png 25254 bytes. UN-AFFECTED INTENTIONALLY: admin panel (internal tool), email templates (image rendering varies across clients), i18n string `appName`, privacy policy body text. All screenshots saved at /tmp/receipt-logo.png, /tmp/legal-terms-backend.png, /tmp/legal-privacy-backend.png, /tmp/pdf-receipt-view.png."
